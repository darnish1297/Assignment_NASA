...
if option == "Approaches":
    ...
    "2. Average velocity of each asteroid over multiple approaches":
        """
        SELECT ca.neo_reference_id, a.name, AVG(ca.relative_velocity_kmph) AS avg_velocity
        FROM close_approach ca
        JOIN asteroids a ON ca.neo_reference_id = a.id
        GROUP BY ca.neo_reference_id, a.name
        """,
    ...
    "3. Top 10 fastest asteroids":
        """
        SELECT ca.neo_reference_id, a.name, MAX(ca.relative_velocity_kmph) AS top_speed
        FROM close_approach ca
        JOIN asteroids a ON ca.neo_reference_id = a.id
        GROUP BY ca.neo_reference_id, a.name
    ...
    "5. Find the month with the most asteroid approaches":
        """
        SELECT MONTH(ca.close_approach_date) AS month, COUNT(*) AS total
        FROM close_approach ca
        GROUP BY month
        ORDER BY total DESC
        LIMIT 1
    ...
    "8. An asteroid whose closest approach is getting nearer over time":
        ...
        FROM close_approach ca
        JOIN asteroids a ON ca.neo_reference_id = a.id
        ORDER BY ca.miss_distance_km ASC, ca.close_approach_date ASC
        """,
    ...
    "11. Count how many approaches happened per month":
        """
        SELECT MONTH(ca.close_approach_date) AS month, COUNT(*) AS approach_count
        FROM close_approach ca
        GROUP BY month
    ...
    "14. Find asteroids that passed closer than the Moon (less than 1 LD)":
        """
        SELECT ca.neo_reference_id, a.name, ca.miss_distance_lunar
        FROM close_approach ca
        JOIN asteroids a ON ca.neo_reference_id = a.id
    ...
    "15. Find asteroids that came within 0.05 AU (astronomical distance)":
        """
        SELECT ca.neo_reference_id, a.name, ca.miss_distance_au
        FROM close_approach ca
        JOIN asteroids a ON ca.neo_reference_id = a.id
